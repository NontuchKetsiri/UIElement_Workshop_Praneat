...
public class Player : MonoBehaviour {
    public BehaviorSubject<float> WalkSpeed = new BehaviorSubject<float>(500);
    public BehaviorSubject<float> JumpForce = new BehaviorSubject<float>(600);
    ...
    void FixedUpdate()
    {
        ...
        if (_inputAxis.x != 0)
        {
            rig.velocity = new Vector2(_inputAxis.x * WalkSpeed.Value * Time.deltaTime, rig.velocity.y);
        ...
        if (_isJump)
        {
            rig.AddForce(new Vector2(0, JumpForce.Value));
            _Legs.clip = _jump;
            _Legs.Play();
            _canJump = false;
            _isJump = false;
